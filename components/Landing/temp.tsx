...
function Card({ active, index, lastChild, children }: CardProps) {
	const styles = {
		activeChild: 'translate-x-0 bg-white rounded-md p-3 drop-shadow-md',
		inactiveChild: '',
		firstChild: '',
		lastChild: '',
	};
...
function Temp() {
	...
	return (
		<div className=" rounded-md flex flex-col space-y-5 items-center relative">
			<ul className=" flex ">
				{cards.map((item, index) => (
					<Card
						active={active}
						index={index}
						lastChild={cards.length - 1}
						key={`Card-${index}`}
					>
						{item}
					</Card>
